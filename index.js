...
function requireAdmin(req, res, next) {
  ...
  const sessionUser = sessions.find(
    (session) => session.id === parseInt(req.body.sessionId)
  );
  if (!sessionUser) {
  ...
  if (!user) {
    return res
      .status(404)
      .send({ error: "SessionId does not have an user associated with it" });
  }
...
app.patch("/times/edit/:id", requireAdmin, (req, res) => {
  ...
  if (
    !req.body.start ||
    !/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(req.body.start)
  ) {
  ...
  if (
    !req.body.end ||
    !/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(req.body.end)
  ) {
...
app.post("/users", (req, res) => {
  ...
  if (user) {
    return res
      .status(409)
      .send({ error: "Conflict: The user already exists. " });
  }
...
app.post("/sessions", (req, res) => {
  ...
  if (!user) {
    return res
      .status(401)
      .send({ error: "Unauthorized: username or password is incorrect" });
  }
  let newSession = {
    id: sessions.length + 1,
    userId: user.id,
